window.PROGRAM_DATA = {
  "version": "5.0",
  "updated_at": "2026-06-22",
  "source": "v5 — restored full program from SKILL.md (A1+A2+B1+B2 + complete mobility)",
  "days": [
    {
      "day_num": 1,
      "color": "teal",
      "title": "يوم 1 — Full Body A",
      "description": "تركيز علوي · مقاومة + Zone 2",
      "duration": "55–60 د",
      "phases": [
        {
          "name": "warmup",
          "label": "🤸 التسخين",
          "exercises": [
            {
              "name": "دراجة 3 دقائق",
              "en": "Bike Warm-up",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "3 د",
              "video": "https://www.youtube.com/watch?v=PjS6hyXfEmI"
            },
            {
              "name": "دوائر الذراعين",
              "en": "Arm Circles",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=PjS6hyXfEmI"
            },
            {
              "name": "تمرير العصا فوق الرأس",
              "en": "PVC Pass Throughs",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=XPdZiOTQNw0"
            },
            {
              "name": "شد الحزام المطاطي",
              "en": "Band Pull-Apart",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 15",
              "video": "https://www.youtube.com/watch?v=smSSXITNpCI"
            },
            {
              "name": "القطة والبقرة",
              "en": "Cat-Cow",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 8–10",
              "video": "https://www.youtube.com/watch?v=Fa4ZMS5M7xA"
            },
            {
              "name": "تسخين نوعي: Machine Chest Press",
              "en": "Specific Warm-up: Chest",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "×15 @ 50% + ×8 @ 70%",
              "video": "https://www.youtube.com/watch?v=Iwe6AmxVf7o"
            }
          ]
        },
        {
          "name": "resistance_a1",
          "label": "🏋️ المقاومة — A1 (أسابيع فردية)",
          "exercises": [
            {
              "name": "Machine Chest Press",
              "en": "جهاز ضغط الصدر جالس",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=Iwe6AmxVf7o"
            },
            {
              "name": "Machine Shoulder Press",
              "en": "ضغط الأكتاف جالس مع دعم ظهر",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=qEwKCR5JCog"
            },
            {
              "name": "Lat Pulldown (Wide Grip)",
              "en": "سحب علوي قبضة عريضة",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=CAwf7n6Luuc"
            },
            {
              "name": "Seated Cable Row (Close Grip)",
              "en": "سحب أرضي قبضة ضيقة",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=GZbfZ033f74"
            },
            {
              "name": "Triceps Pushdown (Rope)",
              "en": "ترايسبس حبل",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "2×12–15",
              "video": "https://www.youtube.com/watch?v=2-LAMcpzODU"
            },
            {
              "name": "Face Pull",
              "en": "سحب للوجه",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "2–3×15–20",
              "video": "https://www.youtube.com/watch?v=repqV8FPoXg"
            },
            {
              "name": "Plank",
              "en": "بلانك",
              "type": "resistance",
              "fields": [
                "time"
              ],
              "default": "3×45 ث",
              "video": "https://www.youtube.com/watch?v=ASdvN_XEl_c"
            }
          ]
        },
        {
          "name": "resistance_a2",
          "label": "🏋️ المقاومة — A2 (أسابيع زوجية)",
          "exercises": [
            {
              "name": "Incline Machine Press",
              "en": "ضغط صدر مائل",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=xfBrP_F3R9Y"
            },
            {
              "name": "Lat Pulldown (Close Grip)",
              "en": "سحب علوي قبضة ضيقة",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=1FCq_RfObcQ"
            },
            {
              "name": "Chest Supported Machine Row",
              "en": "سحب أرضي مدعوم الصدر",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×10–12",
              "video": "https://www.youtube.com/watch?v=j2qo7fXkV_c"
            },
            {
              "name": "Cable Lateral Raise (Single Arm)",
              "en": "رفع جانبي كيبل",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×12–15",
              "video": "https://www.youtube.com/watch?v=PPrzBe7yh_k"
            },
            {
              "name": "Triceps Pushdown (Straight Bar)",
              "en": "ترايسبس مستقيم",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "2×12–15",
              "video": "https://www.youtube.com/watch?v=2-LAMcpzODU"
            },
            {
              "name": "Reverse Pec Deck أو Face Pull",
              "en": "ب pec deck عكسي",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "2×15–20",
              "video": "https://www.youtube.com/watch?v=repqV8FPoXg"
            },
            {
              "name": "Side Plank",
              "en": "بلانك جانبي",
              "type": "resistance",
              "fields": [
                "time"
              ],
              "default": "3×30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=K2VJrW_rRng"
            }
          ]
        },
        {
          "name": "cardio",
          "label": "🫀 الكارديو",
          "exercises": [
            {
              "name": "Zone 2 — مشي منحدر Treadmill",
              "en": "Zone 2 Incline Walk",
              "type": "cardio",
              "fields": [
                "time",
                "hr"
              ],
              "default": "15 د · 110-125 نبضة",
              "video": "https://www.youtube.com/watch?v=OdevRNWoQmM"
            }
          ]
        },
        {
          "name": "cooldown",
          "label": "🧘 الإطالات",
          "exercises": [
            {
              "name": "Doorway Chest Stretch",
              "en": "إطالة صدر",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث",
              "video": "https://www.youtube.com/watch?v=M850sCj9LHQ"
            },
            {
              "name": "Cross-Body Shoulder Stretch",
              "en": "إطالة كتف",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=AWwD5O8PYRo"
            },
            {
              "name": "Triceps Stretch (بدون إجبار)",
              "en": "إطالة ترايسبس",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=meE8b3V2c-U"
            },
            {
              "name": "Lat Stretch",
              "en": "إطالة لاتس",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=eLxXg5VP3yE"
            }
          ]
        },
        {
          "name": "recovery",
          "label": "🧖 التعافي",
          "exercises": [
            {
              "name": "سونا (12-15 د)",
              "en": "Sauna",
              "type": "recovery",
              "fields": [
                "time"
              ],
              "default": "12-15 د · 70-80°م",
              "video": "https://www.youtube.com/watch?v=BLk7-DW8Awk"
            }
          ]
        }
      ]
    },
    {
      "day_num": 2,
      "color": "amber",
      "title": "يوم 2 — HIIT + كارديو",
      "description": "Norwegian 4×4 · VO₂max",
      "duration": "50–55 د",
      "phases": [
        {
          "name": "warmup",
          "label": "🤸 التسخين",
          "exercises": [
            {
              "name": "دراجة تصاعدية",
              "en": "Progressive Bike Warm-up",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "5 د Zone 1 + 3 د Zone 2",
              "video": "https://www.youtube.com/watch?v=PjS6hyXfEmI"
            },
            {
              "name": "Arm Circles",
              "en": "دوائر الذراعين",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=PjS6hyXfEmI"
            },
            {
              "name": "Leg Swings",
              "en": "تأرجح الأرجل",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10/رجل",
              "video": "https://www.youtube.com/watch?v=3HfD3X4Q5zg"
            },
            {
              "name": "Torso Twists",
              "en": "دوران جذع",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=GgRmfpA55kY"
            },
            {
              "name": "Hip Circles",
              "en": "دوائر الورك",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=zbH4XmSREoc"
            }
          ]
        },
        {
          "name": "hiit",
          "label": "⚡ Norwegian 4×4",
          "exercises": [
            {
              "name": "Norwegian 4×4 — 4 د Zone 4 + 3 د تعافي",
              "en": "HIIT 4×4",
              "type": "hiit",
              "fields": [
                "rounds",
                "hr_max"
              ],
              "default": "4 جولات · 154-165 نبضة",
              "video": "https://www.youtube.com/watch?v=OdevRNWoQmM"
            }
          ]
        },
        {
          "name": "cooldown",
          "label": "🫀 تهدئة + إطالة",
          "exercises": [
            {
              "name": "تهدئة دراجة",
              "en": "Cool-down Bike",
              "type": "cardio",
              "fields": [
                "time",
                "hr"
              ],
              "default": "10-15 د · RPE 2-3",
              "video": "https://www.youtube.com/watch?v=PjS6hyXfEmI"
            },
            {
              "name": "إطالة Quads",
              "en": "Quad Stretch",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=2HRE7QF6K1k"
            },
            {
              "name": "إطالة Hamstrings",
              "en": "Hamstring Stretch",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=60FfBPJB1Fg"
            },
            {
              "name": "إطالة Glutes",
              "en": "Glute Stretch",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=VgjgTGnBkx0"
            },
            {
              "name": "إطالة Back",
              "en": "Back Stretch",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث",
              "video": "https://www.youtube.com/watch?v=4E0lvDt2M9o"
            },
            {
              "name": "إطالة Shoulders",
              "en": "Shoulder Stretch",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=AWwD5O8PYRo"
            }
          ]
        },
        {
          "name": "recovery",
          "label": "🧖 التعافي",
          "exercises": [
            {
              "name": "ماء + أملاح + وجبة",
              "en": "Hydration + Meal",
              "type": "recovery",
              "fields": [
                "time"
              ],
              "default": "خلال 30 د",
              "video": ""
            }
          ]
        }
      ]
    },
    {
      "day_num": 3,
      "color": "blue",
      "title": "يوم 3 — Full Body B",
      "description": "تركيز سفلي · أرجل + Core",
      "duration": "55–65 د",
      "phases": [
        {
          "name": "warmup",
          "label": "🤸 التسخين",
          "exercises": [
            {
              "name": "دراجة 3 دقائق",
              "en": "Bike Warm-up",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "3 د",
              "video": "https://www.youtube.com/watch?v=PjS6hyXfEmI"
            },
            {
              "name": "Leg Swings (أمامي/جانبي)",
              "en": "Leg Swings",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10/رجل",
              "video": "https://www.youtube.com/watch?v=3HfD3X4Q5zg"
            },
            {
              "name": "Bodyweight Squats",
              "en": "سكوات وزن جسم",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=aclHkVaku9U"
            },
            {
              "name": "Hip Circles",
              "en": "دوائر الورك",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=zbH4XmSREoc"
            },
            {
              "name": "Walking Lunges (بدون وزن)",
              "en": "Walking Lunges BW",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=D7KaRcUTQeE"
            },
            {
              "name": "Ankle Rotations",
              "en": "دوران الكاحل",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=8X1YQPZ0a-M"
            },
            {
              "name": "تسخين نوعي: Leg Press",
              "en": "Specific Warm-up: Legs",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "×15 @ 50% + ×8 @ 70%",
              "video": "https://www.youtube.com/watch?v=IZxyjW7MPJQ"
            }
          ]
        },
        {
          "name": "resistance_b1",
          "label": "🦵 المقاومة — B1 (أسابيع فردية)",
          "exercises": [
            {
              "name": "Leg Press",
              "en": "ليج برس",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10",
              "video": "https://www.youtube.com/watch?v=IZxyjW7MPJQ"
            },
            {
              "name": "Seated Leg Curl (ثني للخلف)",
              "en": "ليج كيرل جالس",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×10–12",
              "video": "https://www.youtube.com/watch?v=1Tq3QdYUuHs"
            },
            {
              "name": "Walking Lunges (وزن جسم) أو Supported Split Squat",
              "en": "Walking Lunges أو Split Squat",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×8–10/رجل",
              "video": "https://www.youtube.com/watch?v=D7KaRcUTQeE"
            },
            {
              "name": "Seated Calf Raise أو Leg Press Calf Raise",
              "en": "سمانة جالس أو ليج برس",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×15–20",
              "video": "https://www.youtube.com/watch?v=JbyjNymZOt0"
            },
            {
              "name": "Abdominal Crunch Machine",
              "en": "جهاز كرنش البطن",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×15–20",
              "video": "https://www.youtube.com/watch?v=Xyd_fa5zoEU"
            },
            {
              "name": "Cable Pullover",
              "en": "سحب علوي كيبل",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "2×12–15",
              "video": "https://www.youtube.com/watch?v=F6pcc9OqqvM"
            }
          ]
        },
        {
          "name": "resistance_b2",
          "label": "🦵 المقاومة — B2 (أسابيع زوجية)",
          "exercises": [
            {
              "name": "Leg Press (High Foot Placement)",
              "en": "ليج برس مرتفع",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×10–12",
              "video": "https://www.youtube.com/watch?v=IZxyjW7MPJQ"
            },
            {
              "name": "Seated Leg Curl (Single Leg)",
              "en": "ليج كيرل ساق واحدة",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×10–12/رجل",
              "video": "https://www.youtube.com/watch?v=1Tq3QdYUuHs"
            },
            {
              "name": "Leg Extension",
              "en": "ليج إكستنشن",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×12–15",
              "video": "https://www.youtube.com/watch?v=YyvSfVjQeL0"
            },
            {
              "name": "Seated Calf Raise أو Leg Press Calf",
              "en": "سمانة جالس أو ليج برس",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×15–20",
              "video": "https://www.youtube.com/watch?v=JbyjNymZOt0"
            },
            {
              "name": "Pallof Press",
              "en": "بالوف بريس",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "3×10–12/جانب",
              "video": "https://www.youtube.com/watch?v=AH-QGqqYFAY"
            },
            {
              "name": "Single-Arm Seated Cable Row أو Chest Supported Machine Row",
              "en": "سحب أرضي ذراع واحدة",
              "type": "resistance",
              "fields": [
                "weight",
                "reps"
              ],
              "default": "2-3×10–12/جانب",
              "video": "https://www.youtube.com/watch?v=roCP6wCboqo"
            }
          ]
        },
        {
          "name": "cardio",
          "label": "🫀 الكارديو",
          "exercises": [
            {
              "name": "Zone 1 — دراجة (استشفاء نشط)",
              "en": "Zone 1 Bike Recovery",
              "type": "cardio",
              "fields": [
                "time"
              ],
              "default": "10 د",
              "video": "https://www.youtube.com/watch?v=OdevRNWoQmM"
            }
          ]
        },
        {
          "name": "cooldown",
          "label": "🧘 الإطالات",
          "exercises": [
            {
              "name": "Standing Quad Stretch",
              "en": "إطالة أمام الفخذ",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=2HRE7QF6K1k"
            },
            {
              "name": "Seated Hamstring Stretch",
              "en": "إطالة خلف الفخذ",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=60FfBPJB1Fg"
            },
            {
              "name": "Figure-4 Glute Stretch",
              "en": "إطالة Glute",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=VgjgTGnBkx0"
            },
            {
              "name": "Couch Stretch (Hip Flexor)",
              "en": "إطالة Hip Flexor",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "60 ث/جانب",
              "video": "https://www.youtube.com/watch?v=-rsIS-wl-ig"
            },
            {
              "name": "Wall Calf Stretch",
              "en": "إطالة سمانة",
              "type": "stretch",
              "fields": [
                "time"
              ],
              "default": "30 ث/جانب",
              "video": "https://www.youtube.com/watch?v=Y7Wy4nDqe70"
            }
          ]
        },
        {
          "name": "recovery",
          "label": "🧖 التعافي",
          "exercises": [
            {
              "name": "Cold Plunge (اختياري)",
              "en": "Cold Plunge",
              "type": "recovery",
              "fields": [
                "time"
              ],
              "default": "8-10 د · 10-15°م",
              "video": "https://www.youtube.com/watch?v=cZ9j8iCcJZA"
            }
          ]
        }
      ]
    },
    {
      "day_num": 4,
      "color": "purple",
      "title": "يوم 4 — Zone 2 طويل",
      "description": "Active Recovery · أكسدة الدهون",
      "duration": "55–60 د",
      "phases": [
        {
          "name": "warmup",
          "label": "🤸 التسخين",
          "exercises": [
            {
              "name": "دراجة خفيفة Zone 1",
              "en": "Easy Bike Zone 1",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "5 د",
              "video": "https://www.youtube.com/watch?v=OdevRNWoQmM"
            }
          ]
        },
        {
          "name": "cardio_main",
          "label": "🫀 الكارديو الرئيسي",
          "exercises": [
            {
              "name": "Zone 2 — دراجة أو مشي منحدر",
              "en": "Zone 2 Long Cardio",
              "type": "cardio_long",
              "fields": [
                "time",
                "hr",
                "cal"
              ],
              "default": "40-50 د · 103-125 نبضة",
              "video": "https://www.youtube.com/watch?v=OdevRNWoQmM"
            }
          ]
        },
        {
          "name": "mobility",
          "label": "🧘 موبيليتي",
          "exercises": [
            {
              "name": "Hip Circles",
              "en": "دوائر الورك",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=zbH4XmSREoc"
            },
            {
              "name": "Leg Swings",
              "en": "تأرجح الأرجل",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10/رجل",
              "video": "https://www.youtube.com/watch?v=3HfD3X4Q5zg"
            },
            {
              "name": "World's Greatest Stretch",
              "en": "إطالة العظم العظيم",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 5/جانب",
              "video": "https://www.youtube.com/watch?v=-CiWQ2IvY34"
            },
            {
              "name": "Deep Squat Hold (حسب الراحة)",
              "en": "سكوات عميق",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "30-60 ث",
              "video": "https://www.youtube.com/watch?v=0wzrgyAurT8"
            },
            {
              "name": "Cat-Cow",
              "en": "القطة والبقرة",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=Fa4ZMS5M7xA"
            },
            {
              "name": "Thread the Needle",
              "en": "إبرة الخيط",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 8/جانب",
              "video": "https://www.youtube.com/watch?v=z3qhwQ4VSgU"
            },
            {
              "name": "Ankle Mobility",
              "en": "حركية الكاحل",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=8X1YQPZ0a-M"
            }
          ]
        }
      ]
    },
    {
      "day_num": 5,
      "color": "green",
      "title": "يوم 5 — موبيليتي شاملة",
      "description": "تعافي + مرونة + Core",
      "duration": "55–60 د",
      "phases": [
        {
          "name": "warmup",
          "label": "🤸 الإحماء",
          "exercises": [
            {
              "name": "تنفس عميق + حركة خفيفة",
              "en": "Deep Breathing + Light Movement",
              "type": "warmup",
              "fields": [
                "time"
              ],
              "default": "5 د",
              "video": "https://www.youtube.com/watch?v=tEmt1Zn4YQ4"
            }
          ]
        },
        {
          "name": "mobility_upper",
          "label": "🧘 موبيليتي — علوي",
          "exercises": [
            {
              "name": "Neck CARs",
              "en": "دوائر الرقبة",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "30 ث/اتجاه",
              "video": "https://www.youtube.com/watch?v=emf1U9ekXrc"
            },
            {
              "name": "Shoulder CARs",
              "en": "دوائر الكتف",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "5 لفات/كتف",
              "video": "https://www.youtube.com/watch?v=emf1U9ekXrc"
            },
            {
              "name": "PVC Pass Throughs",
              "en": "تمرير العصا",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=XPdZiOTQNw0"
            },
            {
              "name": "Scapular Wall Slides",
              "en": "سحب لوحي على الحائط",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=tUMRozoU4WM"
            },
            {
              "name": "Thoracic Spine Rotation (من وضعية الجلوس)",
              "en": "دوران صدر",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 8/جانب",
              "video": "https://www.youtube.com/watch?v=5DIzPMfgQ9c"
            },
            {
              "name": "Band Pull-Aparts",
              "en": "شد الحزام",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 15",
              "video": "https://www.youtube.com/watch?v=smSSXITNpCI"
            }
          ]
        },
        {
          "name": "mobility_lower",
          "label": "🧘 موبيليتي — سفلي",
          "exercises": [
            {
              "name": "Hip CARs",
              "en": "دوائر الورك",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "5 لفات/ورك",
              "video": "https://www.youtube.com/watch?v=zbH4XmSREoc"
            },
            {
              "name": "Cossack Squat (حسب الراحة)",
              "en": "سكوات كوساك",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 6-8/جانب",
              "video": "https://www.youtube.com/watch?v=W-KbaAOpMhM"
            },
            {
              "name": "World's Greatest Stretch",
              "en": "إطالة العظم العظيم",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 5/جانب",
              "video": "https://www.youtube.com/watch?v=-CiWQ2IvY34"
            },
            {
              "name": "Ankle Dorsiflexion Drill",
              "en": "ثني الكاحل",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10/كاحل",
              "video": "https://www.youtube.com/watch?v=8X1YQPZ0a-M"
            },
            {
              "name": "Deep Squat Hold (حسب الراحة)",
              "en": "سكوات عميق",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "30-60 ث",
              "video": "https://www.youtube.com/watch?v=0wzrgyAurT8"
            },
            {
              "name": "Couch Stretch (Hip Flexor)",
              "en": "إطالة Hip Flexor",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "60 ث/جانب",
              "video": "https://www.youtube.com/watch?v=-rsIS-wl-ig"
            }
          ]
        },
        {
          "name": "core",
          "label": "💪 Core + استقرار",
          "exercises": [
            {
              "name": "Dead Bug",
              "en": "حشرة ميتة",
              "type": "resistance",
              "fields": [
                "time"
              ],
              "default": "3×8/جانب",
              "video": "https://www.youtube.com/watch?v=4E0lvDt2M9o"
            },
            {
              "name": "Bird Dog",
              "en": "كلب طائر",
              "type": "resistance",
              "fields": [
                "time"
              ],
              "default": "3×8/جانب",
              "video": "https://www.youtube.com/watch?v=wiFNA3sqjCA"
            },
            {
              "name": "Cat-Cow",
              "en": "القطة والبقرة",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "× 10",
              "video": "https://www.youtube.com/watch?v=Fa4ZMS5M7xA"
            },
            {
              "name": "Child's Pose",
              "en": "وضعية الطفل",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "60 ث",
              "video": "https://www.youtube.com/watch?v=2lOYWHw4jhc"
            },
            {
              "name": "Foam Rolling: Quads",
              "en": "فوم رولنج أمام الفخذ",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "30-60 ث",
              "video": "https://www.youtube.com/watch?v=sXy6LwdwDcc"
            },
            {
              "name": "Foam Rolling: Hamstrings",
              "en": "فوم رولنج خلف الفخذ",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "30-60 ث",
              "video": "https://www.youtube.com/watch?v=sXy6LwdwDcc"
            },
            {
              "name": "Foam Rolling: Upper Back",
              "en": "فوم رولنج ظهر علوي",
              "type": "mobility",
              "fields": [
                "time"
              ],
              "default": "30-60 ث",
              "video": "https://www.youtube.com/watch?v=sXy6LwdwDcc"
            }
          ]
        },
        {
          "name": "cardio",
          "label": "🫀 كارديو خفيف",
          "exercises": [
            {
              "name": "مشي منحدر Treadmill Zone 1",
              "en": "Incline Walk Zone 1",
              "type": "cardio",
              "fields": [
                "time"
              ],
              "default": "10-15 د",
              "video": "https://www.youtube.com/watch?v=OdevRNWoQmM"
            }
          ]
        },
        {
          "name": "recovery",
          "label": "🧖 التعافي",
          "exercises": [
            {
              "name": "سونا أو بخار",
              "en": "Sauna or Steam",
              "type": "recovery",
              "fields": [
                "time"
              ],
              "default": "12-15 د",
              "video": "https://www.youtube.com/watch?v=BLk7-DW8Awk"
            }
          ]
        }
      ]
    }
  ],
  "health_metrics": [
    {
      "key": "weight",
      "label": "الوزن",
      "unit": "كجم",
      "color": "teal"
    },
    {
      "key": "muscle_mass",
      "label": "الكتلة العضلية",
      "unit": "كجم",
      "color": "blue"
    },
    {
      "key": "fat_pct",
      "label": "نسبة الدهون",
      "unit": "%",
      "color": "red"
    },
    {
      "key": "bmi",
      "label": "BMI",
      "unit": "",
      "color": "purple"
    },
    {
      "key": "vo2max",
      "label": "VO₂max",
      "unit": "ml/kg/min",
      "color": "green"
    },
    {
      "key": "resting_hr",
      "label": "نبض الراحة",
      "unit": "نبضة",
      "color": "amber"
    }
  ],
  "weekly_metrics": [
    {
      "week": 1,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 2,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 3,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 4,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 5,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 6,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 7,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 8,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 9,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 10,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 11,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    },
    {
      "week": 12,
      "weight": null,
      "muscle_mass": null,
      "fat_pct": null,
      "bmi": null,
      "vo2max": null,
      "resting_hr": null
    }
  ]
};
